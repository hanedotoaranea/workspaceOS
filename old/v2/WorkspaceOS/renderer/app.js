...
try {
    const assetsPath = path.join(__dirname, '..', 'assets', 'wallpaper.jpg').replace(/\\/g, '/');
    DEFAULT_WALLPAPER = `file://${assetsPath}`;
} catch (e) {
    console.warn('Не удалось определить путь к обоям:', e);
    DEFAULT_WALLPAPER = ''; 
}

function getRandomGradient(seed) {
  const gradients = [
    'linear-gradient(135deg, #FF9A9E 0%, #FECFEF 100%)', 'linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%)',
    'linear-gradient(135deg, #84fab0 0%, #8fd3f4 100%)', 'linear-gradient(135deg, #fccb90 0%, #d57eeb 100%)',
    'linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%)', 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)',
...
function renderDesktop() {
  ...
  pinnedApps.forEach(pinned => {
    ...
    icon.innerHTML = `<div class="desktop-icon-wrapper" style="background: ${gradient}">${app.icon}</div><div class="desktop-icon-name">${app.name}</div>`;
    
    icon.addEventListener('click', (e) => { if (!dragState.moved) launchApp(app); });
    icon.addEventListener('contextmenu', (e) => { e.preventDefault(); e.stopPropagation(); showContextMenu(e, app.id); });
    icon.addEventListener('mousedown', (e) => {
      if (e.button !== 0) return;
      e.preventDefault(); e.stopPropagation();
      dragState = { active: true, element: icon, appId: app.id, startX: e.clientX, startY: e.clientY, initialLeft: pinned.x, initialTop: pinned.y, moved: false };
...
document.addEventListener('mouseup', (e) => {
  ...
  setTimeout(() => { dragState = { active: false, element: null, appId: null, startX: 0, startY: 0, initialLeft: 0, initialTop: 0, moved: false }; }, 50);
});
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (typeof ipcRenderer !== 'undefined') {
        ipcRenderer.on('windows-updated', async () => {
            if (isSwitcherActive) {
            openWindows = await ipcRenderer.invoke('get-open-windows');
            renderSwitcherCards();
    ...
    document.addEventListener('keydown', async (e) => {
      if (e.altKey && e.key === 'Tab') {
        e.preventDefault();
        if (!isSwitcherActive && typeof ipcRenderer !== 'undefined') {
          openWindows = await ipcRenderer.invoke('get-open-windows');
          toggleWindowSwitcher();
        } else if (isSwitcherActive) {
          currentSwitcherIndex = (currentSwitcherIndex + 1) % (openWindows.length || 1);
          updateSwitcherSelection();
        }
      }
      
      if (e.key === 'Escape') {
        if (isSwitcherActive) toggleWindowSwitcher();
        ...
        if (power && power.classList.contains('active')) power.classList.remove('active');
        const lock = document.getElementById('lockScreen');
        if (lock && lock.classList.contains('active')) unlockScreen();
      }
    });
    ...
    } else if (savedTheme === 'custom-image' && DEFAULT_WALLPAPER) {
    ...
    if (typeof ipcRenderer !== 'undefined') {
        ...
        if (pinnedApps.length === 0 && allApps.length > 0) {
            ...
            defaults.forEach((app, index) => { pinnedApps.push({ id: app.id, x: 40 + index * (ICON_WIDTH + 20), y: 150 }); });
            localStorage.setItem('pinnedApps', JSON.stringify(pinnedApps));
        }
    }
    ...
    console.error('Ошибка инициализации workspaceOS:', err);
  }
});
...
function renderLauncherGrid(filter = '') {
  ...
  if (!grid) return;
  grid.innerHTML = '';
  const filtered = allApps.filter(app => app.name.toLowerCase().includes((filter || '').toLowerCase()));
  filtered.forEach(app => {
    const card = document.createElement('div');
    card.className = 'grid-app-card glass';
    const gradient = getRandomGradient(app.name);
    card.innerHTML = `<div class="grid-app-icon" style="background: ${gradient}">${app.icon}</div><div class="grid-app-name">${app.name}</div>`;
    card.onclick = () => { launchApp(app); toggleLauncher(); };
    grid.appendChild(card);
  });
}

function filterApps(query) { renderLauncherGrid(query); }

async function toggleWindowSwitcher() {
  ...
  if (isSwitcherActive) {
    ...
    if (openWindows[currentSwitcherIndex] && typeof ipcRenderer !== 'undefined') {
...
function renderSwitcherCards() {
  ...
  openWindows.forEach((win, index) => {
    ...
    card.innerHTML = `<div style="font-size:3rem;margin-bottom:10px;">🪟</div><div style="color:white;font-weight:600;text-align:center;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${win.title || 'Без названия'}</div>`;
    card.onclick = () => { currentSwitcherIndex = index; toggleWindowSwitcher(); };
...
function launchApp(app) { 
    if (typeof ipcRenderer !== 'undefined') ipcRenderer.invoke('launch-app', app.exec); 
}
...
function togglePinFromOverview() {
  ...
  if (existing) pinnedApps = pinnedApps.filter(p => p.id !== overviewContextAppId);
  else { const pos = findFreePosition(); pinnedApps.push({ id: overviewContextAppId, x: pos.x, y: pos.y }); }
  localStorage.setItem('pinnedApps', JSON.stringify(pinnedApps));
  renderDesktop();
  renderLauncherGrid(document.querySelector('.launcher-search')?.value || '');
  renderSettingsAppList();
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        if (powerMenu && powerMenu.classList.contains('active')) {
        ...
        // Overview контекстное меню
        const overviewContext = document.getElementById('overviewContextMenu');
        if (overviewContext && overviewContext.classList.contains('active')) {
            if (typeof hideOverviewContextMenu === 'function') {
                hideOverviewContextMenu();
            } else {
                overviewContext.classList.remove('active');
            }
            e.preventDefault();
        }
    }
...
function setTheme(themeName) { currentTheme = themeName; localStorage.setItem('theme', themeName); applyTheme(themeName); updateThemeSelection(); }
function applyTheme(themeName) {
  const themes = { 'blue': 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)', 'dark': 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', 'purple': 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', 'sunset': 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' };
  document.body.style.background = themes[themeName] || themes['purple'];
}
function updateThemeSelection() { document.querySelectorAll('.theme-card').forEach(c => c.classList.remove('active-theme')); const active = document.querySelector(`.theme-card.${currentTheme}`); if (active) active.classList.add('active-theme'); }
function setCustomColor(color) { document.body.style.background = color; localStorage.setItem('theme', 'custom'); updateThemeSelection(); }
function setCustomImage(input) {
  if (input.files && input.files[0]) {
    const reader = new FileReader();
    reader.onload = function(e) { document.body.style.background = `url(${e.target.result}) center/cover no-repeat`; localStorage.setItem('bgImage', e.target.result); localStorage.setItem('theme', 'custom-image'); updateThemeSelection(); };
    reader.readAsDataURL(input.files[0]);
  }
}

function toggleEffect(type, isEnabled) { effects[type] = isEnabled; localStorage.setItem('effects', JSON.stringify(effects)); applyEffects(); }
function applyEffects() {
  if (effects.animations) document.body.classList.remove('no-animations'); else document.body.classList.add('no-animations');
  if (effects.transparency) document.body.classList.remove('no-transparency'); else document.body.classList.add('no-transparency');
}

function openSystemApp(id) { const app = allApps.find(a => a.id === id); if (app) launchApp(app); toggleSettings(); }
...
function renderSettingsAppList() {
  ...
  sortedApps.forEach(app => {
    ...
    item.innerHTML = `<div style="width:30px;height:30px;border-radius:50%;background:${getRandomGradient(app.name)};display:flex;align-items:center;justify-content:center;font-size:1rem;">${app.icon}</div><span>${app.name}</span><span class="pin-indicator">${isPinned ? '' : '+'}</span>`;
...
function togglePin(id) {
  ...
  if (existing) pinnedApps = pinnedApps.filter(p => p.id !== id);
  else { const pos = findFreePosition(); pinnedApps.push({ id, x: pos.x, y: pos.y }); }
  localStorage.setItem('pinnedApps', JSON.stringify(pinnedApps));
  renderDesktop();
  renderSettingsAppList();
}
...
function lockScreen() { 
    ...
    if(ls) { ls.classList.add('active'); updateLockScreen(); }
}
function updateLockScreen() { const now = new Date(); const lt = document.getElementById('lockTime'); const ld = document.getElementById('lockDate'); if(lt) lt.innerText = formatTime(now, showSeconds); if(ld) ld.innerText = formatDate(now); }
function unlockScreen() { const ls = document.getElementById('lockScreen'); if(ls) ls.classList.remove('active'); }
...
function shutdownSystem() {
  ...
  if(confirm('Вы действительно хотите выключить компьютер?')) launchApp({ exec: cmd });
  const powerMenu = document.getElementById('powerMenu');
  if(powerMenu) powerMenu.classList.remove('active');
}

function setDateFormat(format) { dateFormat = format; localStorage.setItem('dateFormat', format); updateDateTime(); }
function applyDateFormat() { const radios = document.querySelectorAll('input[name="dateFormat"]'); radios.forEach(r => { r.checked = (r.value === dateFormat); }); }
function toggleSecondsDisplay(enabled) { showSeconds = enabled; localStorage.setItem('showSeconds', enabled); updateDateTime(); }

function formatDate(now) {
  const days = ['Воскресенье','Понедельник','Вторник','Среда','Четверг','Пятница','Суббота'];
  const daysShort = ['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
  const months = ['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря'];
  const day = now.getDate(); const month = months[now.getMonth()]; const year = now.getFullYear();
  const dayName = days[now.getDay()]; const dayNameShort = daysShort[now.getDay()];
  switch (dateFormat) {
